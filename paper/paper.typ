...
Complementando el análisis convolucional, la arquitectura EfficientNet-B0 se integra como un modelo altamente optimizado que aplica el principio de escalado compuesto (*Compound Scaling*). A diferencia de las redes tradicionales que escalan dimensiones de forma arbitraria, EfficientNet equilibra matemáticamente la profundidad, el ancho de los canales y la resolución de la imagen de entrada. Esta eficiencia estructural permite capturar representaciones espaciales ricas en ecosistemas complejos con una fracción de los parámetros computacionales requeridos por arquitecturas más pesadas @tanEfficientNetRethinkingModel2020.

Para contrastar el paradigma convolucional local, el estudio incorpora el Vision Transformer (ViT-tiny), una arquitectura que prescinde de las convoluciones tradicionales. ViT divide el recorte satelital en secuencias de parches fijos y aplica mecanismos de auto-atención global (*Self-Attention*). Este enfoque le confiere a la red un campo receptivo global desde la primera capa, permitiéndole analizar simultáneamente contextos geográficos distantes dentro de la misma imagen, como la relación entre un cuerpo de agua turbio y un claro deforestado en extremos opuestos de la escena @radVisionTransformerMultispectral2024.

Finalmente, para mitigar la elevada demanda computacional del auto-atención global, se implementa el Swin Transformer (Swin-T). Esta arquitectura introduce un esquema jerárquico basado en ventanas desplazadas (*shifted windows*) que restringe el cálculo de atención a vecindarios locales y progresivamente expande el campo de visión @liuSwinTransformerHierarchical2021. Esta topología reintroduce un sesgo inductivo de localidad, logrando un equilibrio óptimo entre la comprensión del contexto global y el reconocimiento de patrones finos fundamentales en la identificación de minería aluvial.
